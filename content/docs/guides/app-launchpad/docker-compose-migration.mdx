...
![Sealos 的 MySQL 数据库连接信息](./images/sealos-mysql-connection-info.png)

除此之外，还需要手动连接数据库然后创建一个数据库。点击【连接信息】右边的【连接】按钮进入数据库终端：

![Sealos 的 MySQL 数据库连接](./images/sealos-mysql-connection.png)
...
![在 Sealos 数据库终端中执行 CREATE DATABASE 命令](./images/sealos-mysql-terminal-create-command.png)
...
![Sealos 的 WordPress 应用部署](./images/sealos-wordpress-deploy.png)
